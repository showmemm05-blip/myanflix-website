...
/**
 * THE COMPACT MOVIE CARD — poster first, chrome last.
 *
 * Built for the dense /media catalog where six or seven of these share a row:
 * the 2:3 poster IS the card, and the words under it are one title line and
 * one quiet meta line (year · rating for movies, year · episode count for
 * series). Everything else earns its place only on hover — the play disc and
 * the watchlist pin — so a grid of these reads as artwork, not controls.
 *
 * The only always-on overlay is a small gold crown on premium titles: access
 * changes what a click can do, so it can't hide, but at this card size the
 * full text badge would cover a third of the poster — the crown disc carries
 * the same meaning at an accessible label's expense of one glyph.
 *
 * Same interaction contract as the big dossier card it complements: an
 * <article> with a full-card <Link> at z-[1], real controls above it at
 * z-[2+], play gated on access, watchlist only where an id exists. Memoised
 * for the same reason MediaCard is — the catalog re-renders per keystroke
 * with up to sixty of these below it.
 */
function MovieCardImpl({
  ...
  return (
    <article className={cn("group/movie relative isolate flex min-w-0 flex-col", className)}>
      {/* ─ Poster ─ */}
      <div className="relative aspect-2/3 overflow-hidden rounded-xl bg-secondary/60 shadow-e1 ring-1 ring-white/8 transition-[box-shadow] duration-200 ease-out ring-inset group-hover/movie:shadow-e2 group-hover/movie:ring-white/16">
        <Image
          src={posterSrc}
          alt=""
          fill
          sizes={sizes}
          priority={priority}
          className="object-cover transition-transform duration-500 ease-out group-hover/movie:scale-[1.05]"
        />
            ...
            className={cn(
              ...
              saved
                ...
                : cn(
                    ...
                    "hover-device:opacity-0 group-hover/movie:opacity-100 group-focus-within/movie:opacity-100 focus-visible:opacity-100",
                  ),
            )}
          >
        ...
        {/* Play — the hover-only affordance, same pointer-events choreography
            as MediaCard so the disc never swallows the pin beside it. */}
        {canPlay && (
          <div
            className={cn(
              "pointer-events-none absolute inset-0 z-[2] flex items-center justify-center",
              "opacity-0 transition-opacity duration-200 ease-out",
              "group-hover/movie:opacity-100 group-focus-within/movie:opacity-100",
            )}
          >
            <button
              type="button"
              ...
              className="pointer-events-none flex size-10 scale-90 items-center justify-center rounded-full bg-white text-black shadow-[0_10px_30px_rgba(0,0,0,0.5)] transition-transform duration-200 ease-out group-hover/movie:pointer-events-auto group-hover/movie:scale-100 group-focus-within/movie:pointer-events-auto group-focus-within/movie:scale-100 hover:scale-105 focus-visible:scale-100 active:scale-95"
            >
      ...
      <div className="min-w-0 px-0.5 pt-2">
        <h3 className="truncate text-[13px] leading-tight font-medium text-foreground transition-colors duration-150 ease-out group-hover/movie:text-primary sm:text-sm">
          {item.title}
        </h3>
